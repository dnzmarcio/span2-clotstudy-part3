...
ylabel <- "Bederson Neurodeficit score at Day 30"
...
tables$outcome$sex$f <- dt |>
  ...
  select(eos_nds_score, txas_reperfusion) |>
  nt_describe(group = txas_reperfusion,
              labels = data_labels,
              measures_ql = list(helper_perc_count2),
              measures_qt = list(helper_mean_sd, 
...
dt_plot <- dt |>
  filter(!is.na(eos_nds_score)) |>
  mutate(eos_nds_score = droplevels(eos_nds_score)) |>
  droplevels()
...
dm <- dt |>
  mutate(eos_nds_score = as.numeric(eos_nds_score) - 1) |>
  select(eos_nds_score, site, enro_model, enro_sex, 
...
dm_imputed <- dt |> 
  mutate(eos_nds_score = as.numeric(eos_nds_score) - 1) |>
  mutate(eos_nds_score = 
           ifelse(eos_nds_score_conduct == "Yes", eos_nds_score,
                  ifelse(eos_day_diff_srg_death < 1, 
                         max(eos_nds_score, na.rm = TRUE),
                         NA)))|> 
  droplevels() |> 
  select(eos_nds_score, 
...
fit <- with(dt_imputed, pim(eos_nds_score ~ site + 
...
fit <- with(dt_imputed_cl3, pim(eos_nds_score ~ site + 
...
fit <- with(dt_imputed_cl4, pim(eos_nds_score ~ site + 
                                  enro_sex + txas_reperfusion))
...
rm(list=setdiff(ls(), c("tables", 
                        "plots",
                        "pim_results")))
save.image("results/span2_clotstudy_p3_bederson_nds_d30_analysis_mitt.Rdata")