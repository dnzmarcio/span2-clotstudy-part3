...
ylabel <- "Bederson Neurodeficit score at Day 1"
...
tables$feasibility$overall <- dt |>
  select(postop_d1_nds_score_conduct) |>
...
tables$outcome$sex$f <- dt |>
  ...
  select(postop_d1_nds_score, txas_reperfusion) |>
  nt_describe(group = txas_reperfusion,
              labels = data_labels,
              measures_ql = list(helper_perc_count2),
              measures_qt = list(helper_mean_sd, 
...
dt_plot <- dt |>
  filter(!is.na(postop_d1_nds_score)) |>
  mutate(postop_d1_nds_score = droplevels(postop_d1_nds_score)) |>
  droplevels()
...
dm <- dt |>
  mutate(postop_d1_nds_score = as.numeric(postop_d1_nds_score) - 1) |>
  select(postop_d1_nds_score, site, enro_model, enro_sex, 
...
fit <- pim(postop_d1_nds_score ~ site + 
...
dm_imputed <- dt |> 
  mutate(postop_d1_nds_score = as.numeric(postop_d1_nds_score) - 1) |>
  mutate(postop_d1_nds_score = 
           ifelse(postop_d1_nds_score_conduct == "Yes", postop_d1_nds_score,
                  ifelse(eos_day_diff_srg_death < 1, 
                         max(postop_d1_nds_score, na.rm = TRUE),
                         NA)))|> 
  droplevels() |> 
  select(postop_d1_nds_score, 
...
fit <- with(dt_imputed, pim(postop_d1_nds_score ~ site + 
...
fit <- with(dt_imputed_cl3, pim(postop_d1_nds_score ~ site + 
...
fit <- with(dt_imputed_cl4, pim(postop_d1_nds_score ~ site + 
                                  enro_sex + txas_reperfusion))
...
rm(list=setdiff(ls(), c("tables", 
                        "plots",
                        "pim_results")))
save.image("results/span2_clotstudy_p3_bederson_nds_d1_analysis_mitt.Rdata")