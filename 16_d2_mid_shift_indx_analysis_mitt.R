...
ylabel <- "Day 3 Midline Shift Index"
...
ylower = min(dt$p_mri_d2_mid_shift_indx, na.rm = TRUE)
yupper = max(dt$p_mri_d2_mid_shift_indx, na.rm = TRUE)
...
tables$outcome$overall <- dt |>
  select("p_mri_d2_mid_shift_indx") |>
  nt_describe(labels = data_labels,
              measures_ql = list(helper_perc_count2),
              measures_qt = list(helper_mean_sd, 
                                 helper_median_iqr, 
...
tables$outcome$sex$f <- dt |>
  ...
  select("p_mri_d2_mid_shift_indx", txas_reperfusion) |>
  nt_describe(group = txas_reperfusion,
              labels = data_labels,
              measures_ql = list(helper_perc_count2),
              measures_qt = list(helper_mean_sd, 
...
tables$outcome$site$overall <- dt |>
  select("p_mri_d2_mid_shift_indx", site) |>
  nt_describe(group = site,
              labels = data_labels,
              measures_ql = list(helper_perc_count2),
              measures_qt = list(helper_mean_sd, 
...
dm <- dt |>
  select(p_mri_d2_mid_shift_indx, site, enro_model, enro_sex, 
         clot_length, txas_reperfusion) |>
  mutate(p_mri_d2_mid_shift_indx = abs(p_mri_d2_mid_shift_indx)) |>
  droplevels()
...
fit <- pim(p_mri_d2_mid_shift_indx ~ site + 
...
plots$pim$noimputation$interaction <- 
  plot_clotlength_interaction(dt, y = p_mri_d2_mid_shift_indx,
                              ylabel = ylabel, 
...
dm_imputed <- dt |> 
  mutate(p_mri_d2_mid_shift_indx = 
           ifelse(mri_d2_conduct == "Yes", abs(p_mri_d2_mid_shift_indx),
                  ifelse(mri_d2_conduct_rsn == "Animal death", 
                         max(abs(p_mri_d2_mid_shift_indx), na.rm = TRUE),
                         NA)))|> 
  droplevels() |> 
  select(p_mri_d2_mid_shift_indx, 
...
fit <- with(dt_imputed, pim(p_mri_d2_mid_shift_indx ~ site + 
...
fit <- with(dt_imputed_cl3, pim(p_mri_d2_mid_shift_indx ~ site + 
...
fit <- with(dt_imputed_cl4, pim(p_mri_d2_mid_shift_indx ~ site + 
                                  enro_sex + txas_reperfusion))
...
rm(list=setdiff(ls(), c("tables",
                        "plots",
                        "pim_results")))
save.image("results/span2_clotstudy_p3_mri_d2_mid_shift_indx_analysis_mitt.Rdata")
