...
dm <- dt |>
  select(full_span_score, site, enro_model, enro_sex, 
...
dm_imputed <- dt |> 
  mutate(full_span_score = 
           ifelse(behav_d30_conduct == "Yes", full_span_score,
                  ifelse(behav_d30_conduct_rsn == "Animal death", 
                         max(full_span_score, na.rm = TRUE),
                         NA)))|> 
  droplevels() |> 
  select(full_span_score, 
...
fit <- with(dt_imputed, pim(full_span_score ~ site + 
...
fit <- with(dt_imputed_cl3, pim(full_span_score ~ site + 
...
fit <- with(dt_imputed_cl4, pim(full_span_score ~ site + 
                                  enro_sex + txas_reperfusion))
...
save.image("results/span2_clotstudy_p3_full_span_score_d30_analysis_mitt.Rdata")
